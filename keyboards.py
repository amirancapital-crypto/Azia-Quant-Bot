...
def my_alerts_menu(alerts: list) -> InlineKeyboardMarkup:
    ...
    for alert in alerts:
        ...
        buttons.append([InlineKeyboardButton(f"❌ {label}", callback_data=f"alert_del_{alert['id']}")])
...
def portfolio_menu(items: list) -> InlineKeyboardMarkup:
    ...
    for item in items:
        ...
        buttons.append([InlineKeyboardButton(f"🗑 {label}", callback_data=f"portfolio_del_{item['id']}")])
